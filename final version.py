...
data = pd.read_csv("gen3flag.csv", delimiter=',')
...
flags = list(data['Flag'])
isform = list(data['Form'])
formindex = list(data['Base'])

monamount = len(names)

class Pokemon:
    def __init__(self, nam, dexnum, gen, typeone, typetwo, stagefirst, stagelast, leg, mythic, abilone, abiltwo, hidabil, mid, flag, form, base):
...
#aa = Pokemon('hold', 1, 2, 'hel', 'hol', True, True, False, False, 'dod', 'dad', 'did', False, False)
#aab = aa
#aac = copy.deepcopy(aa)
#aad = copy.copy(aa)
#aa = ''
    ...
    if choosefilter == 'name':
        ...
        for b in tosort:
            if input == b.abone:
                ...
                filteredlist.append(b)
    elif choosefilter == 'flag':
        for b in tosort:
            if b.flagged == True:
                filteredlist.append(b)
    elif choosefilter == 'form':
        for b in tosort:
            if b.form == True:
                filteredlist.append(b)
...
def createentry():
    ...
    while counter < monamount:
        newmon = Pokemon(names[counter], dexnumber[counter], gen[counter], primarytype[counter], secondarytype[counter], firststage[counter], fullyevolved[counter], legendary[counter], mythical[counter], ability1[counter], ability2[counter], abilityhidden[counter], middlevo[counter], flags[counter], isform[counter], formindex[counter])
        counter += 1
        dex.append(newmon)
        if newmon.base != -1:
            formlist.append(newmon)
            baseforms.append(dex[newmon.base-2])
...
leaveinstructtwobut = Button(110, 50, 120, 60, "Return", [(screen.get_width() - 110)/2, 390, 110, 50], [(screen.get_width()-120)/2, 385, 120, 60], 'instructwo', 'tomenu')
allbuttons.append(leaveinstructtwobut)
leaveresultbut = Button(100, 50, 110, 60, "Menu", [3*screen.get_width()/7 - 55, 400, 100, 50], [3*screen.get_width()/7 -60, 395, 110, 60], 'results', 'tomenu')
allbuttons.append(leaveresultbut)
rerunbut = Button(100, 50, 110, 60, "Refine search", [5*screen.get_width()/7 - 45, 400, 100, 50], [5*screen.get_width()/7 - 50, 395, 110, 60], 'results', 'rerun')
backbut = Button(100, 50, 110, 60, "Previous", [screen.get_width()/7 - 65, 400, 100, 50], [screen.get_width()/7 - 70, 395, 110, 60], 'results', 'resultback')
forbut = Button(100, 50, 110, 60, "Next", [6*screen.get_width()/7 - 40, 400, 100, 50], [6*screen.get_width()/7 - 45, 395, 110, 60], 'results', 'resultforward')
wipebut = Button(100, 50, 110, 60, "Clear results", [2*screen.get_width()/7 - 60, 400, 100, 50], [2*screen.get_width()/7 - 65, 395, 110, 60], 'results', 'clear')
allbuttons.append(wipebut)
flagbut = Button(100, 50, 110, 60, "Flag results", [4*screen.get_width()/7 - 50, 400, 100, 50], [4*screen.get_width()/7 - 55, 395, 110, 60], 'results', 'flag')
allbuttons.append(flagbut)
instructtwobut = Button(110, 50, 120, 60, "Next", [(screen.get_width() + 165)/2, 390, 110, 50], [(screen.get_width() + 155)/2, 385, 120, 60], 'instructions', 'toinstructwo')
allbuttons.append(instructtwobut)
instructonebut = Button(110, 50, 120, 60, "Back", [(screen.get_width() - 385)/2, 390, 110, 50], [(screen.get_width() - 395)/2, 385, 120, 60], 'instructwo', 'toinstructions')
allbuttons.append(instructonebut)
...
while not game_over:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for button in activebuttons:
                if button.botrect[0] <= x <= button.botrect[0] + button.botrect[2] and button.botrect[1] <= y <= button.botrect[1] + button.botrect[3]:
                    if button.purpose == 'startfilter':
                    ...
                    elif button.purpose == 'toinstructwo':
                        currentscreen = 'instructwo'
                        ...
                        filterstep = 'initialinput'
                        filterfirst = False
                    elif button.purpose == 'flag':
                        for mon in filteredlist:
                            mon.flagged = not mon.flagged
                        filterfirst = True
                        currentscreen = 'menu'
                        filteredlist.clear()
    ...
    if currentscreen == 'menu':
        ...
        screen.blit(subhead_font.render('Form(s)', True, text_colour), (15, 345))
    elif currentscreen == 'instructwo':
        screen.blit(heading_font.render('Instructions', True, text_colour), ((screen.get_width() - heading_font.render('Instructions', True, text_colour).get_width()) / 2, 10))
        screen.blit(subhead_font.render('After a search is completed, you can use the refine search button to perform a search with', True, text_colour), (5, 65))
        screen.blit(subhead_font.render('multiple applied filters, the clear results button to wipe the search criteria and begin a new', True, text_colour), (5, 85))
        screen.blit(subhead_font.render('search from scratch, or use the flag results button. When used, this flags all of the Pokèmon', True, text_colour), (5, 105))
        screen.blit(subhead_font.render('matching the results of the current search.All Pokèmon flagged in this way will show up when', True, text_colour), (5, 125))
        screen.blit(subhead_font.render('the filter term ‘flag’ is used, allowing you to create your own categories to filter through. If', True, text_colour), (5, 145))
        screen.blit(subhead_font.render('the flag results button is used again, any Pokèmon in the current results that are flagged will', True, text_colour), (5, 165))
        screen.blit(subhead_font.render('be unflagged. The search term ’form’ will alter the search so that only Pokèmon with', True, text_colour), (5, 185))
        screen.blit(subhead_font.render('gameplay-altering alternate form will be displayed.', True, text_colour), (5, 205))
        ...
        if filterstep == 'initialinput':
            ...
            if tocheck == 'NAME':
                ...
                whichfilter = 'ability'
            elif 'FLAG' in tocheck or 'TAG' in tocheck:
                whichfilter = 'flag'
            elif 'FORM' in tocheck:
                whichfilter = 'form'
            else:
                whichfilter = 'invalid'
            typed = ''
            ...
            if whichfilter == 'myth' or whichfilter == 'legend' or whichfilter == 'flag' or whichfilter == 'form':
                filterstep = 'processinputtwo'
            screen.blit(subhead_font.render('Which trait would you like to filter by?', True, text_colour), ((5, 60)))
            screen.blit(underline, (5, 112))
            screen.blit(subhead_font.render(initinpu, True, text_colour), (5, 90))
        ...
        colomncount = 0
        todisplay = []
        displayrangebot = resultpage*50
        todisplay.clear()
        undisplayed = filteredlist.__len__() - resultpage*50
        if not isresults:
            screen.blit(subhead_font.render('No Pokèmon found', True, text_colour), (5, 60))
        elif undisplayed > 50:
            for i in range(displayrangebot, (displayrangebot + 50)):
                todisplay.append(filteredlist[i])
                if filteredlist[i] not in seen:
                    seen.append(filteredlist[i])
            ...
            for i in range((filteredlist.__len__() - undisplayed),(filteredlist.__len__())):
                ...
                if filteredlist[i] not in seen:
                    seen.append(filteredlist[i])
        for mon in formlist:
            if baseforms[formlist.index(mon)] in seen and mon in todisplay:
                todisplay.remove(mon)

        for mon in todisplay:
            if colomncount > 4:
                colomncount = 0
                rowcount += 1
            result_font.render(mon.name, True, text_colour)
            screen.blit(result_font.render(mon.name, True, text_colour), (5 + (colomncount*screen.get_width()/5), 60 + 30*rowcount))
            colomncount += 1
        if resultpage > 0:
